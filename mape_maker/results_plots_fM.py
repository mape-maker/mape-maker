...
if __name__ == "__main__":
    ...
    def plot_results_in_f_M(results, M, metric="mare"):
        ...
        for k in results:
            ...
            plt.plot(Ms, moving_mean, marker="x", linestyle="dashed",
                     label="{}".format(metric + " " + k))
        ...
        plt.legend()
        plt.title(
            "Convergence of the simulations {} ({} simulations)".format(metric, M))
        plt.grid(True)
        fname = metric+".png"
        plt.savefig(fname)
    ...
    plot_results_in_f_M(mare_embedder.saved_scores, M,
                        metric="error_auto_correlation")
